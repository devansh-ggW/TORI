...
function initAuth(){
  ...
  google?.addEventListener("click",async()=>{google.disabled=true;google.textContent="OPENING GOOGLE…";const {error}=await client.auth.signInWithOAuth({provider:"google",options:{redirectTo:TORI_SITE_URL+"/auth.html",queryParams:{prompt:"select_account"}}});if(error){google.disabled=false;google.textContent="CONTINUE WITH GOOGLE";setMsg(error.message||"Google sign-in could not start.","warn")}});
